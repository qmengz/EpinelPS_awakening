...
namespace EpinelPS.LobbyServer.Inventory
{
    ...
    public class AwakeningResetOption : LobbyMsgHandler
    {
        ...
        private static List<int> GetLockKeys(AwakeningOption option, int keyId)
        {
            ...
            if (keyId != 1)
            {
                lockKeys.AddRange([.. dic.Where(x => x.Value.StateEffectGroupId == GetStateEffectGroupid(option.Option1Id)).Select(x => x.Key)]);
            }
            // if (option.Option2Lock || option.IsOption2DisposableLock)
            if (keyId != 2)
            {
                lockKeys.AddRange([.. dic.Where(x => x.Value.StateEffectGroupId == GetStateEffectGroupid(option.Option2Id)).Select(x => x.Key)]);
            }
            // if (option.Option3Lock || option.IsOption3DisposableLock)
            if (keyId != 3)
            {
                lockKeys.AddRange([.. dic.Where(x => x.Value.StateEffectGroupId == GetStateEffectGroupid(option.Option3Id)).Select(x => x.Key)]);
            }
