...
namespace EpinelPS.LobbyServer.Inventory
{
    ...
    public class AwakeningUpgradeOption : LobbyMsgHandler
    {
        ...
        private static int GetOptionId(AwakeningOption option)
        {
            ...
            {
                foreach (StateEffectList effect in record.StateEffectList)
                {
                    stateEffectIds.Add(effect.StateEffectId);
                }
            }
        ...
        private static List<EquipmentOptionRecord> GetOptionDic(AwakeningOption option)
        {
            ...
            {
                return [.. GameData.Instance.EquipmentOptionTable.Where(x => x.Value.StateEffectGroupId == GetStateEffectGroupid(option.Option1Id)).Select(x => x.Value)];
            }
            else if (!option.Option2Lock && !option.IsOption2DisposableLock)
            {
                return [.. GameData.Instance.EquipmentOptionTable.Where(x => x.Value.StateEffectGroupId == GetStateEffectGroupid(option.Option2Id)).Select(x => x.Value)];
            }
            else if (!option.Option3Lock && !option.IsOption3DisposableLock)
            {
                return [.. GameData.Instance.EquipmentOptionTable.Where(x => x.Value.StateEffectGroupId == GetStateEffectGroupid(option.Option3Id)).Select(x => x.Value)];
            }
            return [];
        }
        ...
        private static int GetStateEffectGroupid(int optionId)
        {
            ...
            {
                foreach (StateEffectList effect in record.Value.StateEffectList)
                {
                    if (effect.StateEffectId == optionId)
                    {
                        return record.Value.StateEffectGroupId;
                    }
                }
            }
            return 0;
