...
namespace EpinelPS.LobbyServer.Inventory
{
    ...
    public class UseSelectBox : LobbyMsgHandler
    {
        protected override async Task HandleAsync()
        {
            ...
            if (GameData.Instance.ItemSelectOptionTable.TryGetValue(itemConsume.UseId, out ItemSelectOptionRowRecord? selectOptions))
            {
                foreach (var selected in req.Select)
                {
                    // find matching option
                    // SelectOption option = selectOptions.SelectOption[selected.Id] ?? throw new Exception($"invalid selection ID {selected.Id}");
                    // Logging.WriteLine($"UseSelectBox {box.ItemType}: Found selected option - {JsonConvert.SerializeObject(option)}", LogType.Debug);

                    // for (int i = 0; i < selected.Count; i++)
                    // {
                    //     if (option.SelectType.Equals("Currency"))
                    //     {
                    //         RewardUtils.AddSingleCurrencyObject(user, ref reward, (CurrencyType)option.SelectId, option.SelectValue);
                    //     }
                    //     else
                    //     {
