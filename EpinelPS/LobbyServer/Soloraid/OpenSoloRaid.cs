...
public class OpenSoloRaid : LobbyMsgHandler
{
    protected override async Task HandleAsync()
    {
        ...
        if (GameData.Instance.SoloRaidManagerTable.TryGetValue(req.RaidId, out SoloRaidManagerRecord? manager))
        {
            ...
            var statEnhance = presetData != null ? GameData.Instance.MonsterStatEnhanceTable.Values.Where(m => m.Lv == presetData.MonsterStageLv && m.GroupId == statEnhanceId) : null;
            if (statEnhance != null)
            {
